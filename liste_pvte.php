...
//PARAMETRAGE CAISSE
	...
	$sql_pvte = "SELECT `pvte_id`, `pvte_code`, `pvte_lib`, `pvt_contact`, `pvt_adresse` FROM `point_vente` WHERE pvte_id<>6";
	$list = $pdo->prepare($sql_pvte);
    $list->execute();
                            ...
                            <div class="portlet light portlet-fit portlet-datatable bordered">
                                <div class="portlet-title">
                                    <div class="caption">
                                                               ...
                                                               <a class="btn green" href="<?= $_SESSION['proflis']?>.php?p=add_pvte"><i class="fa fa-plus"></i>Ajouter</a>
...
<?php foreach ($list as $key => $result): ?>
            ...
            <tr>
                ...
                <td><?= $result['pvte_code'] ?></td>
                <td><?= $result['pvte_lib'] ?></td>
                <td><?= $result['pvt_contact'] ?></td>
                <td>
                   <?= $result['pvt_adresse'] ?>
                </td>
               <td align="left" nowrap="nowrap"><a href="<?= $_SESSION['proflis']?>.php?p=up_pvte&pvte_id=<?php echo crypturl($result['pvte_id']); ?>" class="btn btn-outline btn-circle btn-sm blue">
