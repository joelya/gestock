...
//liste des services
	...
	$sql_svc = "SELECT * FROM `service` INNER JOIN direction WHERE service.dir_id = direction.dir_id AND service.flag=1 ORDER BY direction.dir_lib ASC";
	$list = $pdo->prepare($sql_svc);
    $list->execute();
...
if(con==true){
location.href='page.php?p=up_svc&svc_ids='+isbn;
//location.replace('?pages=paramettre&tp='+isbn);
}
else{
location.href='page.php?p=liste_svc';
return false;
}
}
</script>
                            ...
                            <div class="portlet light portlet-fit portlet-datatable bordered">
                                <div class="portlet-title">
                                    <div class="caption">
                                                               ...
                                                               <a class="btn green" href="<?= $_SESSION['proflis']?>.php?p=add_svc"><i class="fa fa-plus"></i>Ajouter</a>
...
<?php foreach ($list as $key => $result): ?>
            ...
            <tr>
                ...
                <td><?= ucfirst(strtolower($result['dir_lib'])) ?></td>
               <td>
                   <?= ucfirst(strtolower($result['svc_lib'])) ?>
                </td>
                <td>
                   <?= $result['svc_date_crea'] ?>
                </td>
                <td>
                	<?= $result['svc_date_mod'] ?>
                </td>
               
                                               <td align="left" nowrap="nowrap"><a href="<?= $_SESSION['proflis']?>.php?p=up_svc&svc_id=<?php echo $result['svc_id']; ?>" class="btn btn-outline btn-circle btn-sm blue">
