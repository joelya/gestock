...
function suggest(element, table, field){
	...
	if(element.value != '' && element.value != value_suggested){
		...
		XHR.open("GET", 'autocomplete_medi.php?table='+table+'&field='+field+'&search='+element.value+'&nocache='+Math.floor((new Date()).getTime()), true); //timestamp en parametre pour empecher la mise en cache
