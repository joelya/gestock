...
if (!empty($prd_id)) {
                $condition.=' AND  p.prd_id='.$prd_id;
             }
			 if (!empty($pvte_id)) {
                $condition.=' AND  pv.pvte_id='.$pvte_id;
             }
$historique = "SELECT p.prd_lib,pv.pvte_lib,SUM(p.prd_prix_ttc) as ca2 FROM vendre_produit vp JOIN vente v ON vp.vente_id = v.vente_id JOIN produit p ON vp.prd_id = p.prd_id JOIN authentification a ON v.auth_id = a.auth_id JOIN point_vente pv ON a.pvte_id = pv.pvte_id WHERE vente_flag_actif=1 AND 1=1 ".$condition." GROUP BY p.prd_id,pv.pvte_id ORDER BY SUM(p.prd_prix_ttc) DESC";
...
<!-- BEGIN PAGE BAR -->
	...
	<div class="page-bar">
                        <ul class="page-breadcrumb">
                            <li>
                             ...
                             <li>
                              <a href="#"><span class="caption"> Historique des ventes par produit</span></a>
                               <i class="fa fa-circle"></i>
                            </li>
  ...
  <form action="" method="post" class="form-horizontal">                                          
                                                <!-- BEGIN FORM-->
													...
													<div class="row"> 
													    <div class="col-md-4">	
															  <label class="control-label">Selectionnez un produit : </label>
															  <select class="bs-select form-control" data-live-search="true" data-size="8" name="prd_id" id="prd_id" >
...
<?php if (isset($_POST['valid_info'])){ if($count_historique>0){ ?>															  <a href="#" class="btn default" onclick="NewWindow('print_histpr.php?prd_id=<?= crypturl($_POST['prd_id']) ?>&pvte_id=<?= crypturl($_POST['pvte_id']); ?>','sous_form_cai',screen.width,screen.height,'yes','center',1);" title="Aper&ccedil;u de la Vente">&nbsp;&nbsp;Aperçu &nbsp;&nbsp;</a><br/><br/><br/>
<?php }} ?>														</div>                
													</div>
												</div>
												</div>
